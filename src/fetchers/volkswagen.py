...
_FEED_URL = "https://jobs.volkswagen-group.com/sitemal.xml"
_G_NS = "http://base.google.com/ns/1.0"


@dataclass
class VolkswagenConfig:
    """Configuration for the Volkswagen Group careers RSS feed fetcher."""

    company: str = "Volkswagen"


class VolkswagenFetcher(BaseFetcher):
    """Fetches job postings from the Volkswagen Group SAP SuccessFactors RSS feed.

    Uses the Google Base feed (sitemal.xml) which includes descriptions inline —
    no separate detail fetch needed.
    """

    def __init__(self, config: VolkswagenConfig) -> None:
        self.config = config
